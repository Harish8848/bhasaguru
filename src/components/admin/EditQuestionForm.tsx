...
import { getMediaRequirements } from "@/lib/question-mapping";
...
export default function EditQuestionForm({
  ...
  const initialMatchingPairs =
    question.type === "MATCHING" && question.options?.pairs
      ? question.options.pairs
      : [{ left: "", right: "" }];

  const initialBlanks =
    question.type === "FILL_BLANK" && question.options?.blanks
  ...
  const [formData, setFormData] = useState({
    ...
    followUpQuestions: Array.isArray(question.followUpQuestions)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formData.type === "MULTIPLE_CHOICE") {
        formattedOptions = formData.options.map(
      ...
      const response = await fetch(
        `/api/admin/mock-test/questions?id=${question.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const removeFollowUp = (index: number) => {
    const newQuestions = formData.followUpQuestions.filter(
      (_: any, i: number) => i !== index
    );
    setFormData((prev) => ({ ...prev, followUpQuestions: newQuestions }));
  };
        ...
        return (
          <>
            <div className="space-y-2">
              <Label htmlFor="questionPassage">
                Context Passage / Script (Optional)
              </Label>
              <Textarea
                id="questionPassage"
                value={formData.questionPassage}
                onChange={(e) =>
                  handleInputChange("questionPassage", e.target.value)
                }
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  handleInputChange("questionSubSection", value)
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  <SelectItem value="listening_part_1">
                    Listening Part 1
                  </SelectItem>
                  <SelectItem value="listening_part_2">
                    Listening Part 2
                  </SelectItem>
                  <SelectItem value="listening_part_3">
                    Listening Part 3
                  </SelectItem>
                  <SelectItem value="listening_part_4">
                    Listening Part 4
                  </SelectItem>
                  <SelectItem value="reading_passage_1">
                    Reading Passage 1
                  </SelectItem>
                  <SelectItem value="reading_passage_2">
                    Reading Passage 2
                  </SelectItem>
                  <SelectItem value="reading_passage_3">
                    Reading Passage 3
                  </SelectItem>
                  <SelectItem value="writing_task_1">Writing Task 1</SelectItem>
                  <SelectItem value="writing_task_2">Writing Task 2</SelectItem>
                  <SelectItem value="speaking_part_1">
                    Speaking Part 1
                  </SelectItem>
                  <SelectItem value="speaking_part_2">
                    Speaking Part 2
                  </SelectItem>
                  <SelectItem value="speaking_part_3">
                    Speaking Part 3
                  </SelectItem>
                </SelectContent>
              </Select>
        ...
        return (
          <>
            ...
            {(formData.questionSubSection === "reading" ||
              formData.questionSubSection === "grammar") && (
              <div className="space-y-2">
  ...
  return (
    ...
    >
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="LISTENING_COMPREHENSION">
                Listening Comprehension
              </SelectItem>
              {(testLanguage?.toLowerCase() === "english" ||
                testType.toUpperCase() === "IELTS") && (
                <>
                  <SelectItem value="SPEAKING_PART1">
                    Speaking Part 1
                  </SelectItem>
                  <SelectItem value="SPEAKING_PART2">
                    Speaking Part 2
                  </SelectItem>
                  <SelectItem value="SPEAKING_PART3">
                    Speaking Part 3
                  </SelectItem>
                </>
              )}
      ...
      <div className="space-y-4">
        <Label>
          Media{" "}
          {getMediaRequirements(testType, formData.type).audioRequired
            ? "(Required)"
            : "(Optional)"}
        </Label>
        ...
        {getMediaRequirements(testType, formData.type).audioRequired && (
          <div className="space-y-2">
            <Label htmlFor="audioUrl" className="text-red-500">
              Audio File *
            </Label>
            <FileUpload
              accept={["audio/*"]}
        ...
        {getMediaRequirements(testType, formData.type).videoAllowed && (
          <div className="space-y-2">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Add the correct word(s) for each blank in the question text.
              </p>
            </div>
            <Button
              type="button"
          ...
          {formData.type === "SPEAKING_PART2" && (
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleInputChange("cueCardContent", e.target.value)
                }
